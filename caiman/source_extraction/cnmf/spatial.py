    ...
    for k_batch in trange(nmf_params['max_iter'], unit='batch'):
        ...
        if err >= last_err:
            ...
            if n_no_improve >= nmf_params['max_no_improvement']:
                logger.info(f'Stopping after {k_batch + 1} epochs due to no improvement')
                break
        else:
            n_no_improve = 0
            ...
            if last_comps is not None:
                ...
                if comps_change < nmf_params['tol']:
                    logger.info(f'Stopping after {k_batch + 1} epochs due to change < tolerance ({nmf_params["tol"]})')
                    break
